...
| `PROG-048 Released [SHIFT] key` | running in T1/T2, switch to AUTO |
| `INTP-106 Cannot execute program` | the program is stuck `PAUSED` with an uncleared error; `ABORT` first, then `RESET` |
| `MCTL-003 System is in error state` | press `RESET`; if it persists, check the e-stop and fault state |
...
| No `KAREL Programs` under `Type` | the R632 option isn't installed, see [prerequisites](prerequisites.md) |
...
`AUX` → `ABORT`, then rerun MAPPDK following [running it](running.md).
