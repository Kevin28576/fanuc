...
> [!IMPORTANT]
> **免責聲明**：這是一個第三方開源 driver，用官方的 KAREL/TP 方法實作，
> 跟 FANUC Corporation 沒有任何隸屬、維護、授權、贊助或背書關係。FANUC
> 以及本文提到的其他商標，其權利都屬於各自的商標持有人所有。

> [!WARNING]
> 基本條件下，搭配套件內建的安全機制（動作／執行程式指令都要靠
> `--confirm` 才會真的執行、動作前的合法性檢查）通常可以預防大多數
> 意外發生，但在某些情況或設置下，仍然可能出現非預期的狀況。強烈
> 建議一定要先在 ROBOGUIDE 虛擬控制器上開發、測試，確認邏輯沒問題
> 之後，再把 `host` 換成控制器 IP 接上真實的機器人，避免程式邏輯上
> 的失誤直接在實機上發生無法預期的狀況。
